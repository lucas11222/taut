// plugin made by @ImShyMike :3
// TODO: port to some kind of nice class-based plugin-loading system

const API_URL = 'https://hackatime.hackclub.com/api/admin/v1/execute'
const API_TOKEN = 'hka_***' // TODO: config file for secrets
...
let trustLevels: Record<string, number> = {}
 ...
 */
function saveTrustLevelsToCache(data: Record<string, number>) {
  try {
...
async function fetchTrustLevelsFromAPI() {
  ...
  try {
    ...
    const trustedButtons: NodeListOf<HTMLButtonElement> = document.querySelectorAll(
 ...
 */
async function setTrustLevel(btn: HTMLButtonElement, slackId: string): Promise<void> {
 ...
 * @param {number} trust
 */
function renderTrust(btn: HTMLButtonElement, trust: number) {
 ...
 * @returns {Promise<void>}
 */
async function handleTrustHover(event: MouseEvent): Promise<void> {
...
/** @param {MouseEvent} event */
function handleTrustLeave(event: MouseEvent) {
  setTimeout(() => {
 ...
 */
async function fetchAuditLogs(slackId: string): Promise<Record<string, any>[] | null> {
  try {
 ...
 * @returns {Promise<void>}
 */
async function showAuditLogsTooltip(btn: HTMLButtonElement, slackId: string): Promise<void> {
...
/** @param {DOMRect} rect */
function createLoadingTooltip(rect: DOMRect) {
...
/** @param {DOMRect} rect */
function createNoLogsTooltip(rect: DOMRect) {
  const tooltip = document.createElement('div')
  tooltip.className = 'trust-audit-tooltip'
  tooltip.style.cssText = `
        position: fixed;
 ...
 */
function createAuditLogsTooltip(rect: DOMRect, logs: Record<string, any>[]) {
  const tooltip = document.createElement('div')
  tooltip.className = 'trust-audit-tooltip'
  tooltip.style.cssText = `
        position: fixed;
  ...
  const logsHtml = logs
    .map((/** @type {Record<string, any>} */ log: Record<string, any>) => {
      const prevLevel =
        /** @type {string|undefined} */ log.previous_trust_level?.[1]
      const newLevel = /** @type {string|undefined} */ log.new_trust_level?.[1]
...
function extractSlackUsers() {
  /** @type {NodeListOf<HTMLButtonElement>} */
  const senderButtons: NodeListOf<HTMLButtonElement> = document.querySelectorAll(
